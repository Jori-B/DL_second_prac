...
##################################################################################################################################
    ...
    dataset_test = tf.data.Dataset.from_tensor_slices((list_of_files, label_test))
    dataset_test = dataset_test.map(audio_file_to_tensor)
    test_dataset = dataset_test.cache().shuffle(len(list_of_files)).batch(BATCH_SIZE).repeat()
        ...
        #    print(id)
            ...
            for code in list_CODE_subfolders_with_paths:
                ...
                for filename in glob.iglob(f"{id_path}/{id}/{code}/*.*", recursive=True):
                    ...
                    label = nationality_per_id.get_nationality(id)
                    labels.append(label)
    label_trans_val = label_encoder.fit(labels)  # fit a transformer that transforms label strings to numerical
    label_val = label_trans_val.transform(labels)
    dataset_val = tf.data.Dataset.from_tensor_slices((list_of_files, label_val))
    dataset_val = dataset_val.map(audio_file_to_tensor)
    val_dataset = dataset_val.cache().shuffle(len(list_of_files)).batch(BATCH_SIZE).repeat()
    val_dataset = test_dataset.prefetch(buffer_size=100)
...
#    embeddings.shape.assert_is_compatible_with([None, 128])
    ...
    print("model 1 ##############################################################################################################")
    model2.summary()
    history = model2.fit(train_dataset, epochs=num_epoch, steps_per_epoch=STEPS_PER_EPOCH,
