...
def preprocess_dataset(files, commands, cache):
    ...
    return model
def define_model(input_shape, ds_train, num_labels, layer_sizes: List[int], dropouts: List[float]):
    ...
    for i in range(len(layer_sizes)):
        model.add(layers.Conv2D(layer_sizes[i], kernel_size=3, activation='relu', padding='same'))
...
def run_model(model, train_ds, val_ds, test_ds, run: int):
    ...
    plt.savefig(output_path + "/{}.png".format(run))
...
def run_experiment(run: int, layer_sizes: List[int], dropouts: List[float]):
    train_ds, val_ds, test_ds, num_labels, input_shape = get_data()
    model = define_model(input_shape, train_ds, num_labels, layer_sizes, dropouts)
...
if __name__ == "__main__":
    ...
    # layer_sizes = [468, 382, 460, 259, 121]
    # dropouts = [0.2919730311857663, 0.039063760165707756, 0.13662771879710645, 0.31877653336772604, 0.110499813458384]

    layer_sizes = [468, 382, 460, 259]
    dropouts = [0.2919730311857663, 0.039063760165707756, 0.13662771879710645, 0.31877653336772604]


    run_experiment(1, layer_sizes, dropouts)
